...
def get_loaders(num_workers, args):
    ...
    if hasattr(args, 'dataset_dir'):
        path_train = f'{args.dataset_dir}/train'
        path_val = f'{args.dataset_dir}/val'
    else:
        path_train = op.expanduser(f'~/data/datasets/images/{args.dataset}/train')
        path_val = op.expanduser(f'~/data/datasets/images/{args.dataset}/val')
