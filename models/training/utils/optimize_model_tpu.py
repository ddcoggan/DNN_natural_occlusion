...
def train_model_tpu(CFG):
    ...
    from models.utils import config_to_text
    config_to_text(CFG)
    ...
    # image processing
    from models.utils import get_transforms
    ...
    if 'contrastive' in T.learning:
        from models.utils import ContrastiveLoss
        ...
        for train_eval in train_evals:
            ...
            with tqdm(loader, unit=f"batch({T.batch_size})") as tepoch:
                ...
                for batch, (inputs, targets) in enumerate(tepoch):
                    ...
                    if 'contrastive' in T.learning:
                        from models.utils import AlterImages, get_remaining_transform
                    ...
                    if epoch == 1 and batch == 0:
                        from models.utils import save_image_custom
